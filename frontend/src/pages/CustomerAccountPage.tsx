  ...
  if (error || !account) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-12 text-center">
        ...
        <Link to="/warehouse/customers" className="mt-4 inline-block text-sm text-boss-green hover:underline">
          Back to directory
        </Link>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 font-mono text-boss-green">{customer.shipping_id}</p>
      ...
      {payablePackages.length > 0 ? (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-boss-green/30 bg-boss-green/5 px-4 py-3">
          <p className="text-sm">
      ...
      {customsPackages.length > 0 ? (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button className="!text-xs" onClick={() => setReleaseOpen(true)}>
              Release &amp; bill
            </Button>
          </div>
        </div>
                ...
                <tbody className="divide-y divide-border">
                  {packages.map((pkg) => (
                    <tr key={pkg.id}>
                      ...
                      <td className="px-4 py-3 font-mono text-boss-green">{pkg.tracking_number}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="rounded-lg border border-border px-2.5 py-1 text-xs font-semibold hover:border-boss-green/40"
                          >
                            Manage
                          </button>
                          {packageEligibleForPayment(pkg) && (
                            <button
                              ...
                              className="rounded-lg border border-boss-green/30 bg-boss-green/10 px-2.5 py-1 text-xs font-semibold text-boss-green hover:bg-boss-green/20"
                            >
                              Pay
                            </button>
                          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="font-mono font-bold text-boss-green">{payPkg.tracking_number}</h2>
