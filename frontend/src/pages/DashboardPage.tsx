  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <p className="mt-2 text-muted">
        Your BOSS shipping ID: <strong className="text-foreground">{user?.shipping_id}</strong>
      </p>

      <div className="mt-8 rounded-2xl border border-boss-green/30 bg-card p-8 shadow-sm">
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-lg font-bold uppercase tracking-wide text-boss-green">
            Your Miami Shipping Address
          </h2>
          {usingCache && (
        ...
        {address ? (
          <pre className="mt-6 whitespace-pre-wrap rounded-lg border border-border bg-background p-6 font-mono text-sm leading-relaxed text-foreground">
            {address.formatted}
          </pre>
        ) : (
          <p className="mt-6 text-muted">
            {offline ? 'No cached address available. Connect to load your Miami address.' : 'Loading address...'}
          </p>
        )}
      ...
      <div className="mt-8">
          ...
          {preAlertError && (
            <p className="mt-3 rounded-lg bg-red-500/10 px-4 py-2 text-sm text-red-400">{preAlertError}</p>
          )}
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold uppercase tracking-wide">My Packages</h2>
        ...
        {packages.length === 0 ? (
          <p className="mt-4 rounded-xl border border-border bg-card p-6 text-sm text-muted">
            No packages yet. Once your shipment is received at the Miami warehouse, it will appear here.
          </p>
        ) : (
          <div className="mt-4 space-y-3">
            {packages.map((pkg) => (
              ...
              >
                ...
                {pkg.shipping_cost_usd != null && (
                  <p className="mt-2 text-sm text-muted">
                    {pkg.billable_weight_lbs} lbs · ${pkg.shipping_cost_usd.toFixed(2)} USD
                  </p>
                )}
              </Link>
            ))}
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          <p className="text-xs uppercase tracking-wider text-muted">TRN on file</p>
          <p className="mt-1 font-semibold">{user?.trn_masked || '***-***-***'}</p>
        </div>
      </div>
    </div>
  )
