...
def _targeted_package_query(criteria: dict):
    query = (
        Package.query.join(User, Package.customer_id == User.id)
        .filter(
...
def _targeted_visibility_filter(user: User | None):
    """SQL filter: broadcast announcements for everyone; targeted only for recipients."""
    if user is None:
...
def _run_email_broadcast(app, job_id: uuid.UUID) -> None:
    with app.app_context():
        ...
        if announcement.target_mode == "targeted":
            ...
            email_targets = [
                (user, [])
                for user in _recipient_query(announcement.audience).all()
            ]
